...
uses
  ...
  cxDataUtils, cxDataStorage, cxFilter{$IFDEF CLR}, System.Collections{$ENDIF};
...
type
  ...
  EcxInvalidDataControllerOperation = class(Exception);

  { TcxCustomDataSource }

  TcxDataRecordHandle = {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF};
  TcxDataItemHandle = {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF};
  ...
  protected
    ...
    function GetInfoForCompare(ARecordHandle: TcxDataRecordHandle; AItemHandle: TcxDataItemHandle; var PValueBuffer: PChar): Boolean; virtual;
  ...
  protected
    ...
    // Data
    function AddRecordHandle(AData: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}): Integer;
...
//    function GetRecordCount: Integer; virtual;
  ...
  protected
    procedure CorrectCount(ARelationCount: Integer);
  public
    constructor Create;
    destructor Destroy; override;
    procedure Clear;
  ...
  TcxDataSortInfo = {$IFNDEF CLR}record{$ELSE}class{$ENDIF}
...
{$ENDIF}
  ...
  TcxDataGroupInfo = {$IFNDEF CLR}record{$ELSE}class{$ENDIF}
    RowIndex: Integer;
    Expanded: Boolean;
    Level: Integer;
...
{$IFNDEF CLR}
  PcxDataGroupInfo = ^TcxDataGroupInfo;
{$ELSE}
  PcxDataGroupInfo = TcxDataGroupInfo;
{$ENDIF}
  ...
  private
    ...
    function GetItem(Index: Integer): PcxDataGroupInfo;
    function GetItemCount: Integer;
    function GetRowCount: Integer;
    function GetRowInfo(ARowIndex: Integer): TcxGroupsRowInfo;
    function GetTopVisibleItem(ARowIndex: Integer; var ACurIndex: Integer): PcxDataGroupInfo;
    procedure Rebuild;
  protected
    procedure Add(const ADataGroupInfo: TcxDataGroupInfo);
    procedure AddField(AField: TcxCustomDataField);
    function AddEmpty: PcxDataGroupInfo;
    procedure Clear;
    function Find(ARowIndex: Integer; var PItem: PcxDataGroupInfo): Integer;
    ...
    property Items[Index: Integer]: PcxDataGroupInfo read GetItem; default;
  ...
  TcxDataExpandingInfo = {$IFNDEF CLR}record{$ELSE}class{$ENDIF}
    Level: Integer;
    Value: Variant;
...
{$ENDIF}
  ...
  TcxDataSelectionInfo = {$IFNDEF CLR}record{$ELSE}class{$ENDIF}
    Level: Integer;
...
{$ENDIF}
  ...
  private
    ...
    function CompareRecords(ARecord1, ARecord2: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}): Integer;
    ...
    procedure QuickSortRecords(ASortList: {$IFNDEF CLR}PPointerList{$ELSE}ArrayList{$ENDIF}; L, R: Integer);
    procedure SaveInfo;
  ...
  private
    function GetDataController: TcxCustomDataController;
  public
    procedure Load(AItemIndex: Integer); virtual;
    property DataController: TcxCustomDataController read GetDataController;
  end;
  ...
  public
    ...
    function AddItem(AParent: TcxFilterCriteriaItemList; AItemLink: TObject;
      ...
      const ADisplayValue: string): TcxFilterCriteriaItem; override;
  ...
  TcxSummaryOption = (soNullIgnore, soSelectedRecords);
  ...
  TcxDataSummaryItems = class(TCollection)
  private
  ...
  public
    ...
    function Add: TcxDataSummaryItem;
    procedure Assign(Source: TPersistent); override;
    procedure AssignEvents(Source: TPersistent); virtual;
    function DefaultFormat(AVarType: TVarType; ASummaryKind: TcxSummaryKind; AIsFooter: Boolean): string; virtual;
  ...
  TcxDataSummaryGroupItemLinks = class(TCollection)
  private
  ...
  published
    property Links: TcxDataSummaryGroupItemLinks read FItemLinks write SetItemLinks;
    property SummaryItems: TcxDataGroupSummaryItems read FSummaryItems write SetSummaryItems;
  end;

  TcxDataSummaryGroups = class(TCollection)
  private
    FSummary: TcxDataSummary;
  ...
  protected
    procedure DoFocusedRecord(AFilteredRecordIndex: Integer);
    function DoSearch(AStartFilteredRecordIndex, AEndFilteredRecordIndex: Integer; const ASubText: string; AForward: Boolean): Integer;
  public
    constructor Create(ADataController: TcxCustomDataController);
    procedure Cancel;
    function Locate(AItemIndex: Integer; const ASubText: string): Boolean;
    function LocateNext(AForward: Boolean): Boolean;
    procedure Lock;
    procedure Unlock;
    property DataController: TcxCustomDataController read FDataController;
  ...
  TcxDataControllerChange = (dccLayout, dccData, dccFocus, dccSelection,
    dccDetail, dccSummary, dccSearch, dccSorting, dccGrouping, dccSyncMasterPost,
  ...
  TcxDataControllerOption = (dcoAnsiSort, dcoCaseInsensitive, dcoAssignGroupingValues,
    ...
    dcoFocusTopRowAfterSorting, dcoGroupsAlwaysExpanded);
  ...
  TcxCustomDataController = class(TPersistent{$IFNDEF CLR}, IUnknown{$ENDIF})
  private
    FActive: Boolean;
...
//    FDataChangeInfo: TcxDataChangeInfo;
  ...
  {$ENDIF}
    ...
    class function GetItemID(AItem: TObject): Integer; virtual;
    ...
    class procedure RemoveListenerLink(var ALink: TcxDataListenerLink);
...
//    function GetDisplayText(ARecordIndex, AItemIndex: Integer): string; virtual;
    ...
    // Compare
    function CompareByField(ARecordIndex1, ARecordIndex2: Integer; AField: TcxCustomDataField): Integer; virtual;
    function CompareEqualRecords(ARecordIndex1, ARecordIndex2: Integer): Integer; virtual;
    function CompareRecords(ARecordIndex1, ARecordIndex2: Integer; ASortInfo: TcxDataSortInfo): Integer; virtual;
    ...
    function LoadRecord(AData: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}): Integer; virtual;
  ...
  {$ENDIF}  
    ...
    procedure AddItem(AItem: TObject); virtual;
    ...
    // Data Changed notify
    procedure AddDataChangedListener(ADataChangedEvent: TNotifyEvent);
    procedure RemoveDataChangedListener(ADataChangedEvent: TNotifyEvent);
    procedure AddDataChangeRefCount;
    procedure RemoveDataChangeRefCount;
    function DataChangedNotifyLocked: Boolean;
    ...
    // TODO: methods (if Grouping then only Detail)
    //function GetSelectedRecordCount: Integer;
    //function GetSelectedRecordIndex(Index: Integer): Integer;
    //procedure ChangeRecordSelection(ARecordIndex: Integer; ASelection: Boolean); virtual;
...
type
  ...
  TcxKeyInfo = {$IFNDEF CLR}record{$ELSE}class{$ENDIF}
    Key: Variant;
  end;
{$IFNDEF CLR}
...
{$ENDIF}
  ...
  TcxSelectedKeyInfo = {$IFNDEF CLR}record{$ELSE}class{$ENDIF}
    Key: Variant;
...
{$ENDIF}
  ...
  TcxDetailKeyInfo = {$IFNDEF CLR}record{$ELSE}class{$ENDIF}
    Key: Variant;
...
{$ENDIF}
  ...
  protected
    ...
    procedure FreeKeyInfo(P: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}); virtual;
  ...
  protected
    ...
    procedure FreeKeyInfo(P: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}); override;
  end;
...
function CompareSelectionList(AItem1, AItem2: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}): Integer;
begin
  Result := PcxDataSelectionInfo(AItem1).RowIndex - PcxDataSelectionInfo(AItem2).RowIndex;
end;

function CompareIntegers(AItem1, AItem2: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}): Integer;
begin
  Result := Integer(AItem1) - Integer(AItem2);
end;

function CompareByKey(AItem1, AItem2: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}): Integer;
begin
...
begin
  ...
  begin
    P := PcxKeyInfo(Keys[I]);
    if VarEquals(DataController.GetInternalRecordId(ARecordIndex, FKeyFields), P.Key) then
    begin
      AIndex := I;
      Result := True;
      Break;
...
procedure TcxKeys.FreeKeyInfo(P: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF});
begin
end;
        ...
        begin
          ...
          begin
            ...
            if ADataController.InheritsFrom(DataController.ClassType) then //if ADataController is TcxDBDataController then
              PDetailKeyInfo.SubDetail.Restore(ADataController);
          end;
        end;
      end;
    end;
...
begin
  ...
  begin
    ...
    {$IFNDEF CLR}
      New(PDetailKeyInfo);
    {$ELSE}
      PDetailKeyInfo := PcxDetailKeyInfo.Create;
    {$ENDIF}
      ...
      begin
        ADataController := DataController.GetDetailDataControllerByLinkObject(ADetailLinkObject);
        if ADataController.InheritsFrom(DataController.ClassType) then //if ADataController is TcxDBDataController then
          PDetailKeyInfo.SubDetail.Save(ADataController);
      end;
    end;
  end;
end;

procedure TcxDetailKeys.FreeKeyInfo(P: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF});
begin
...
begin
  ...
  // notify: Expanding -> RecalcSelection + CorrectPrevSelectionChangedInfo
...
procedure TcxSelectedKeys.FreeKeyInfo(P: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF});
begin
{$IFNDEF CLR}
  Dispose(PcxSelectedKeyInfo(P));
{$ELSE}
  PcxSelectedKeyInfo(P).Free;
{$ENDIF}
end;
...
begin
  Result := DataController.RecordCount > 0;
end;
  ...
  try
    ...
    begin
      ...
      if ARowInfo.Level < DataController.Groups.LevelCount then // It's Group Row
        ...
        AList.Add({$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}(ARowInfo.RecordIndex));
    end;
    AList.Sort(CompareIntegers);
    // Prepare List - delete duplicates
...
procedure TcxCustomDataProvider.Post;
var
  ARecordIndex: Integer;
  APostFlag: Boolean;
begin
  ...
  if IsInserting and not IsModified then
    Cancel
  else
  begin
    APostFlag := True;
  ...
  DataController.DataChanged(dcRecord, -1, ARecordIndex); // TODO: !!!
  if APostFlag then
...
begin
  ...
  if not IsDataSource and not IsInserting then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FEditingRecordIndex2 := ARecordIndex + 1;
        end;
      end;
    end;
...
function TcxCustomDataProvider.AddRecordHandle(AData: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}): Integer;
begin
  Result := -1;
  if CustomDataSource <> nil then
  begin
...
function TcxCustomDataProvider.NativeCompare(ARecordIndex1, ARecordIndex2: Integer;
  ...
  procedure GetCompareInfo(ARecordIndex: Integer; var IsNull: Boolean; var P: PChar);
  var
    ARecordHandle: TcxDataRecordHandle;
    AItemHandle: TcxDataItemHandle;
  begin
...
begin
  ...
  begin
    if (FCustomDataSource <> nil) and (FCustomDataSource.FProvider = Self) then
      FCustomDataSource.FProvider := nil;
    FCustomDataSource := Value;
    if FCustomDataSource <> nil then
    begin
      if FCustomDataSource.FProvider = nil then
        FCustomDataSource.FProvider := Self;
    end;
...
procedure TcxDetailObject.CorrectCount(ARelationCount: Integer);
begin
  if FInfoObjects.Count < ARelationCount then
    FInfoObjects.Count := ARelationCount;
    ...
    begin
      ...
      begin
        ...
        DataController.SetStoredValue(ARecordIndex, FDataField,
          {$IFNDEF CLR}Integer{$ELSE}Variant{$ENDIF}(Result));
      end;
      Exit;
    end;
    if Result = nil then
...
begin
  ...
  if VarIsNull(AValue) then
    ...
    Result := TcxDetailObject({$IFNDEF CLR}Integer{$ENDIF}(AValue));
end;
...
{$ELSE}
  PcxDataSortInfo(FItems[Index]).Free;
{$ENDIF}
  FItems.Delete(Index);
  Changed;
end;
...
begin
  Result := PcxDataSortInfo(FItems[Index]){$IFNDEF CLR}^{$ENDIF};
end;
...
var
  PItem: PcxDataGroupInfo;
  I: Integer;
begin
  if (Find(ARowIndex, PItem) <> -1) and Assigned(PItem) then
  begin
    PItem.Expanded := AExpanded;
    if ARecursive and (PItem.Level < (LevelCount - 1)) then
    begin
      for I := PItem.BeginRecordListIndex to Count - 1 do
        if Items[I].Level > PItem.Level then
...
var
  ...
  PItem: PcxDataGroupInfo;
begin
  Result := Find(ARowIndex, PItem);
  if (Result = -1) and (PItem <> nil) then // if it is a data row, go to the nearest top group row
  begin
    ARowIndex := PItem.RowIndex;
    Result := Find(ARowIndex, PItem);
  end;
...
function TcxDataGroups.AddEmpty: PcxDataGroupInfo;
begin
{$IFNDEF CLR}
  New(Result);
{$ELSE}
  Result := PcxDataGroupInfo.Create;
{$ENDIF}
  FItems.Add(Result);
end;
...
var
  I: Integer;
  PInfo: PcxDataGroupInfo;
begin
  FFields.Clear;
  for I := 0 to FItems.Count - 1 do
  begin
    PInfo := PcxDataGroupInfo(FItems[I]);
  {$IFNDEF CLR}
    Dispose(PInfo);
  {$ELSE}
    PInfo.Free;
...
function TcxDataGroups.Find(ARowIndex: Integer; var PItem: PcxDataGroupInfo): Integer;
var
  L, H, I, C: Integer;
begin
  PItem := nil;
  ...
  if L <= H then
    repeat
      I := (L + H) div 2;
      PItem := Items[I];
      C := PItem.RowIndex - ARowIndex;
      if C = 0 then // It's Group
      begin
        PItem := GetTopVisibleItem(ARowIndex, I);
        Result := I;
        Break;
      end
      else
...
begin
  ...
  begin
    ...
    if PItem.Level = ALevel then
      AAnchor := I;
    if (PItem.Level = ALastLevel) and
      (PItem.BeginRecordListIndex <= ARecordListIndex) and
      (ARecordListIndex <= PItem.EndRecordListIndex) then  // only for Data rows 
    begin
      Result := AAnchor;
      Break;
    end;
...
function TcxDataGroups.MakeVisible(AIndex: Integer; AExpand: Boolean): Boolean;
  ...
  procedure ExpandItem(PItem: PcxDataGroupInfo);
  begin
    if not PItem.Expanded then
    begin
      PItem.Expanded := True;
      Result := True;
    end;
...
function TcxDataGroups.GetItem(Index: Integer): PcxDataGroupInfo;
begin
  Result := PcxDataGroupInfo(FItems[Index]);
end;
...
begin
  ...
  begin
    ...
    if PItem.Expanded then
      Inc(Result, PItem.EndRecordListIndex - PItem.BeginRecordListIndex + 1); // only for Data rows
  end
  else
    Result := 0;
end;
...
{$ENDIF}
  if Find(ARowIndex, PItem) <> -1 then // Group
  begin
    Result.Level := PItem.Level;
    Result.Expanded := PItem.Expanded;
    while PItem.Level < (LevelCount - 1) do
      PItem := Items[PItem.BeginRecordListIndex];
    Result.RecordListIndex := PItem.BeginRecordListIndex;
  end
  else
    if PItem <> nil then
    begin
      ...
      Result.Expanded := False;
      Result.RecordListIndex := PItem.BeginRecordListIndex + (ARowIndex - PItem.RowIndex - 1);
    end;
end;

function TcxDataGroups.GetTopVisibleItem(ARowIndex: Integer; var ACurIndex: Integer): PcxDataGroupInfo;
var
  I, J: Integer;
  AMaxLevel: Integer;
  PItem: PcxDataGroupInfo;
begin
  ...
  repeat
    PItem := Items[I];
    if PItem.RowIndex <> ARowIndex then
      Break;
    // check level
    if PItem.Level < AMaxLevel then
    begin
      AMaxLevel := PItem.Level;
      if not PItem.Expanded then
        J := I;
    end;
...
var
  ...
  begin
    ...
    begin
      PItem := Items[ACurIndex];
      if PItem.Level = ALevel then
      begin
        PItem.RowIndex := ARowIndex;
        if AVisible and PItem.Expanded and (PItem.Level = (LevelCount - 1)) then
          Inc(ARowIndex, PItem.EndRecordListIndex - PItem.BeginRecordListIndex + 1)
        else
        begin
          if AVisible and PItem.Expanded then
            Inc(ARowIndex);
          SubItems(PItem.Level + 1, AVisible and PItem.Expanded);
        end;
      end
      else
      begin
...
begin
  ...
  {$ELSE}
    PcxDataExpandingInfo(FItems[I]).Free;
  {$ENDIF}
  FItems.Clear;
end;
...
begin
  ...
  {$ELSE}
    PcxDataSelectioninfo(FItems[I]).Free;
  {$ENDIF}
  FItems.Clear;
end;
...
begin
  FChanges := FChanges + [dcicLoad, dcicSummary];
...
begin
  ...
  if soSelectedRecords in DataController.Summary.Options then
    FChanges := FChanges + [dcicSummary];
end;
...
begin
  ...
  begin
    ...
    begin
      AGroupIndex := DataGroups.Find(ARowIndex, PItem);
...
begin
  if ARowIndex <> -1 then
    ...
    Result := DataController.CanFocusRecord(-1); // TODO: method?
end;
...
type
  TcxDataGroupCompareInfo = {$IFNDEF CLR}record{$ELSE}class{$ENDIF}
    PInfo: PcxDataGroupInfo;
    RecordIndex: Integer;
  end;
{$IFNDEF CLR}
...
var
  ...
  begin
    ...
    {$IFNDEF CLR}
      New(PCompareInfo);
    {$ELSE}
      PCompareInfo := PcxDataGroupCompareInfo.Create;
    {$ENDIF}
      PCompareInfo.RecordIndex := ARecordIndex;
      PCompareInfo.PInfo := DataGroups.AddEmpty;
      PCompareInfo.PInfo.RowIndex := ARowIndex;
      PCompareInfo.PInfo.Expanded := False;
      PCompareInfo.PInfo.Level := I;
      if I < (FDataGroups.LevelCount - 1) then
        PCompareInfo.PInfo.BeginRecordListIndex := I + 1
      else
        PCompareInfo.PInfo.BeginRecordListIndex := 0;
  ...
  begin
    ...
    begin
      PcxDataGroupCompareInfo(ACurLevels[ALevelIndex]).PInfo.EndRecordListIndex := ARecordListIndex - 1;

      PcxDataGroupCompareInfo(ACurLevels[ALevelIndex]).PInfo := DataGroups.AddEmpty;
      PcxDataGroupCompareInfo(ACurLevels[ALevelIndex]).PInfo.RowIndex := ARowIndex;
      PcxDataGroupCompareInfo(ACurLevels[ALevelIndex]).PInfo.Expanded := False;
      PcxDataGroupCompareInfo(ACurLevels[ALevelIndex]).PInfo.Level := ALevelIndex;
      PcxDataGroupCompareInfo(ACurLevels[ALevelIndex]).PInfo.BeginRecordListIndex := ARecordListIndex;
      PcxDataGroupCompareInfo(ACurLevels[ALevelIndex]).RecordIndex := ARecordIndex;
    end
    else
    begin
      PcxDataGroupCompareInfo(ACurLevels[ALevelIndex]).PInfo.EndRecordListIndex := DataGroups.Count - 1;

      PcxDataGroupCompareInfo(ACurLevels[ALevelIndex]).PInfo := DataGroups.AddEmpty;
      PcxDataGroupCompareInfo(ACurLevels[ALevelIndex]).PInfo.RowIndex := ARowIndex;
      PcxDataGroupCompareInfo(ACurLevels[ALevelIndex]).PInfo.Expanded := False;
      PcxDataGroupCompareInfo(ACurLevels[ALevelIndex]).PInfo.Level := ALevelIndex;
      PcxDataGroupCompareInfo(ACurLevels[ALevelIndex]).PInfo.BeginRecordListIndex := DataGroups.Count;
      PcxDataGroupCompareInfo(ACurLevels[ALevelIndex]).RecordIndex := ARecordIndex;
  ...
  begin
    ...
    begin
      PcxDataGroupCompareInfo(ACurLevels[I]).PInfo.EndRecordListIndex := AEndRecordListIndex;
      if I = (FDataGroups.LevelCount - 1) then
        AEndRecordListIndex := DataGroups.Count - 1
      else
        AEndRecordListIndex := PcxDataGroupCompareInfo(ACurLevels[I]).PInfo.BeginRecordListIndex - 1;
    end;
  end;
  ...
  begin
    ...
    {$ELSE}
      PcxDataGroupCompareInfo(ACurLevels[I]).Free;
    {$ENDIF}
    ACurLevels.Free;
  end;
...
var
  I: Integer;
begin
if FRecordList.Count <> DataController.RecordCount then // TODO: remove
  FRecordList.Count := DataController.RecordCount;
  for I := 0 to FRecordList.Count - 1 do
    FRecordList[I] := {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}(I);
end;
...
begin
  ...
  begin
    PrepareSorting;
    try
      QuickSortRecords(FRecordList.List, 0, FRecordList.Count - 1);
    finally
      UnprepareSorting;
    end;
  end;
...
procedure TcxCustomDataControllerInfo.DoSummary;
begin
  if [csDestroying, csReading{csLoading}] * DataController.FOwner.ComponentState = [] then // TODO: method IsLoading/Destroying
...
function TcxCustomDataControllerInfo.FindFocusedRow(ANearest: Boolean): Integer;
  ...
  begin
    ...
    begin
      I := AGroupIndex;
      PItem := DataGroups.GetTopVisibleItem(DataGroups[AGroupIndex].RowIndex, I);
      ...
      begin
        ...
        begin
          Result := PItem.RowIndex;
          Inc(Result, ARecordListIndex - PItem.BeginRecordListIndex + 1);
          Exit;
        end
        else
          Dec(ALevel);
...
begin
  Result := FRecordList.IndexOf({$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}(ARecordIndex));
end;
...
procedure TcxCustomDataControllerInfo.SaveExpanding(ASaveStates: TcxDataExpandingInfoStateSet);
  ...
  procedure AddGroup(ADataGroupInfo: PcxDataGroupInfo; AGroupIndex: Integer; AExpanded: Boolean);
  var
    ARecordIndex: Integer;
    PInfo: PcxDataGroupInfo;
  ...
  begin
    // begin proc
    PInfo := ADataGroupInfo;
    while PInfo.Level < (DataGroups.LevelCount - 1) do
      PInfo := DataGroups[PInfo.BeginRecordListIndex];
    ARecordIndex := GetInternalRecordIndex(PInfo.BeginRecordListIndex);
    // end proc
    if ARecordIndex < DataController.RecordCount then
    begin
      AStateSet := [];
      if AExpanded and not IsAlwaysExpanded then
  ...
  var
    ...
    ADataGroupInfo: PcxDataGroupInfo;
  begin
    I := 0;
    AVisibleLevel := 0;
...
begin
  ...
  if ARecordCount = 0 then
    ...
    if DataController.FInLoadStorage and not DataController.IsKeyNavigation then
    begin
...
begin
  ...
  ASummaryChanged := dcicSummary in FChanges;
...
var                                   
    ...
    try
      ...
      try
        ...
        begin
          ...
          AValueDefReader.SetValue(FExpandingInfo.Fields[ALocateObject.Level].ValueDef,
            PExpandingInfo.Value);

          ALocateObject.ReadData(AValueDefReader);
          ARecordIndex := ALocateObject.FindRecordIndex;
          if (ARecordIndex <> -1) and (PExpandingInfo.State <> []) then
  ...
  procedure RebuildSelection;
    ...
    procedure CheckGroup(ADataGroupInfo: PcxDataGroupInfo);
    begin
...
begin
  ...
  begin
    ...
    if (0 <= FFocusingInfo.RowIndex) and (FFocusingInfo.RowIndex < GetRowCount) and
      ...
      //ANewFocusedRowIndex := FFocusingInfo.RowIndex;
    else
...
begin
  Result := DataController.CompareRecords(ARecordIndex1, ARecordIndex2,
    FTotalSortingFieldList[AIndex]);
end;

function TcxCustomDataControllerInfo.CompareRecords(ARecord1, ARecord2: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}): Integer;
var
  I, AI, A1, A2: Integer;
begin
  Result := 0;
...
procedure TcxCustomDataControllerInfo.QuickSortRecords(
  ASortList: {$IFNDEF CLR}PPointerList{$ELSE}ArrayList{$ENDIF};
  L, R: Integer);
var
  I, J: Integer;
  P, T: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF};
begin
  repeat
    I := L;
    J := R;
    ...
    repeat
      while CompareRecords(ASortList[I], P) < 0 do
        Inc(I);
      while CompareRecords(ASortList[J], P) > 0 do
        Dec(J);
      if I <= J then
      begin
        T := ASortList[I];
    ...
    until I > J;
    if L < J then
      QuickSortRecords(ASortList, L, J);
    L := I;
  until I >= R;
end;
...
begin
  ASaveStates := [];
  if dcoSaveExpanding in DataController.Options then
  begin
    if ([dcicLoad, dcicGrouping] * FChanges <> []) then
...
var
  ...
  begin
    ...
    begin
      ...
      V := DataController.GetFilterDataValue(I, AField);
      if Criteria.ValueIsNull(V) then
        ANullExist := True
      else
        Add(fviValue, V, DataController.GetFilterDisplayText(I, AItemIndex), False);
    end;
  end;
...
//  SortByDisplayText := DataController.GetItemValueSource(AItemIndex) <> evsText; // TODO: Text for Lookup Field!!!
  ...
  Add(fviAll, Null,
    {$IFNDEF CLR}cxSFilterString(@cxSFilterBoxAllCaption){$ELSE}cxSFilterBoxAllCaption{$ENDIF},
    False);
  Add(fviCustom, Null,
    {$IFNDEF CLR}cxSFilterString(@cxSFilterBoxCustomCaption){$ELSE}cxSFilterBoxCustomCaption{$ENDIF},
    False);
  if DataController.IsGridMode then
    ANullExist := True // custom loading
  else
  begin
    ANullExist := False;
    LoadRecords;
  ...
  begin
    Add(fviBlanks, Null,
      {$IFNDEF CLR}cxSFilterString(@cxSFilterBoxBlanksCaption){$ELSE}cxSFilterBoxBlanksCaption{$ENDIF},
      False);
    Add(fviNonBlanks, Null,
      {$IFNDEF CLR}cxSFilterString(@cxSFilterBoxNonBlanksCaption){$ELSE}cxSFilterBoxNonBlanksCaption{$ENDIF},
      False);
  end;
...
function TcxDataFilterCriteria.AddItem(AParent: TcxFilterCriteriaItemList; AItemLink: TObject;
  AOperatorKind: TcxFilterOperatorKind; const AValue: Variant;
...
begin
  ...
  Result := inherited AddItem(AParent, AItemLink, AOperatorKind, AValue, ADisplayValue);
end;

procedure TcxDataFilterCriteria.Assign(Source: TPersistent);
begin
...
begin
  DataController.CheckItemRange(AItemIndex);
  Result := DataControllerInfo.GroupingFieldList.GroupIndexByField(DataController.Fields[AItemIndex]);
end;
...
begin
  ...
  begin
    ...
    begin
      S := Format;
      if Format = '' then
        S := SummaryItems.DefaultFormat(VarType(AValue), Kind, AIsFooter);
...
function TcxDataSummaryItems.DefaultFormat(AVarType: TVarType; ASummaryKind: TcxSummaryKind;
...
var
  AVarIsDate: Boolean;  
begin
  if ASummaryItem.DataField = nil then Exit;
...
begin
  Result := '';
  ADataGroupIndex := DataController.DataControllerInfo.DataGroups.GetIndexByRowIndex(RowIndex);
  if ADataGroupIndex <> -1 then
  begin
    ...
    if (ASummaryItems <> nil) and (PSummaryValues <> nil) and not VarIsEmpty(PSummaryValues{$IFNDEF CLR}^{$ENDIF}) then
      Result := ASummaryItems.GetGroupText(PSummaryValues{$IFNDEF CLR}^{$ENDIF});
  end;
end;
...
begin
  ...
  if (PSummaryValues <> nil) and IsValidSummaryValuesIndex(PSummaryValues{$IFNDEF CLR}^{$ENDIF}, Index) then
    Result := PSummaryValues{$IFNDEF CLR}^{$ENDIF}[Index]
  else
    Result := Null;
end;
...
begin
  ...
  begin
    ...
    if (ASummaryItems <> nil) and (PSummaryValues <> nil) and
      IsValidSummaryValuesIndex(PSummaryValues{$IFNDEF CLR}^{$ENDIF}, Index) then
...
begin
  ...
  begin
    ...
    begin
      FSetCustomSummary := True;
      try
        PSummaryValues{$IFNDEF CLR}^{$ENDIF}[Index] := Value;
        Changed(True);
      finally
        FSetCustomSummary := False;
      end;
...
begin
  ...
  FDataController.DataStorage.ReadRecord(FInternalRecordIndex, AValueDefReader, True);
end;
...
begin
  ...
  FSearch := TcxDataControllerSearch.Create(Self);
...
procedure TcxCustomDataController.AddItem(AItem: TObject);
var
  AField: TcxCustomDataField;
begin
...
procedure TcxCustomDataController.AddDataChangedListener(ADataChangedEvent: TNotifyEvent);
var
  E: TNotifyEventItem;
begin
...
procedure TcxCustomDataController.RemoveDataChangedListener(ADataChangedEvent: TNotifyEvent);
var
  I: Integer;
  E: TNotifyEventItem;
begin
  ...
  begin
    ...
    if @E.Event = @ADataChangedEvent then
    begin
      E.Free;
...
begin
  Result := False;
  CheckItemRange(AItemIndex);
  if Provider.CanModify and Fields[AItemIndex].CanModify(GetItemValueSource(AItemIndex)) then
...
procedure TcxCustomDataController.Post;
var
...
begin
  ...
  try
    ...
    if S <> stDataControllerSignature then
...
//    if AValueCount > 0 then
    begin
      if DataStorage.ValueDefs.Count <> AValueCount then
        InvalidOperation(cxSDataInvalidStreamFormat);
      // ValueDefs
      for I := 0 to AValueCount - 1 do
      begin
        ReadStringProc(AStream, S);
        if DataStorage.ValueDefs[I].ValueTypeClass.ClassName <> S then
          InvalidOperation(cxSDataInvalidStreamFormat);
      end;
      BeginUpdate;
      try
...
begin
  ...
  try
    AWriter.WriteString(stDataControllerSignature);
    // ValueCount
    if not IsProviderMode then
      AValueCount := DataStorage.ValueDefs.Count
    else
      AValueCount := 0;
...
//    if AValueCount > 0 then
    begin
      ...
      for I := 0 to DataStorage.ValueDefs.Count - 1 do
        AWriter.WriteString(DataStorage.ValueDefs[I].ValueTypeClass.ClassName);
      // RecordCount
...
//      if FMasterRelation.FDetailDataController <> nil then
//        CustomDataSource := FMasterRelation.FDetailDataController.CustomDataSource;
    end;
      ...
      for I := 0 to DataStorage.RecordCount - 1 do
        ...
        begin
          if AIndex = (ARelationIndexes.Count - 1) then
            AProc(GetDetailDataController(I, ARelationIndex))
          else
            ForEach(GetDetailDataController(I, ARelationIndex), AIndex + 1);
        end;    
    end;
  end;
...
begin
  // TODO: DB!!!
  AIsBound := IsProviderMode or Provider.IsDataSource;
...
var
  ...
  begin
    ...
    while (AForward and (I <= AEndIndex)) or
      (not AForward and (I >= AStartIndex)) do
    begin
...
begin
  ...
  try
    ...
    AFocusedRowIndex := GetFocusedRowIndex;
    if AFocusedRowIndex <> -1 then
      SelectRows(AFocusedRowIndex, AFocusedRowIndex);
  finally
    EndUpdate;
  end;
end;
...
var
  ARecordIndex: Integer;
begin
  if GetFocusedRecordIndex = -1 then
  begin
...
begin
  NewItemRecordIndex;
  if not (((0 <= ARecordIndex) and (ARecordIndex < RecordCount)) or (ARecordIndex = FNewItemRecordIndex)) then
    InvalidOperation(cxSDataRecordIndexError);
end;
...
begin
  ...
  try
    if IsProviderMode and (AToRecordIndex >= 0) then
      ...
      DataStorage.ReadRecord(AToRecordIndex, AValueDefReader, True);
  finally
    AValueDefReader.Free;
  end;
...
class function TcxCustomDataController.GetItemID(AItem: TObject): Integer;
begin
  Result := -1;
end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          if Provider.IsGridModeUpdating and (EditState * [dceInsert] <> []) then // appending record
            // do nothing
          else
            UpdateFields;
        end;
...
class procedure TcxCustomDataController.RemoveListenerLink(var ALink: TcxDataListenerLink);
begin
  if ALink.Ref <> nil then
...
function TcxCustomDataController.CompareRecords(ARecordIndex1, ARecordIndex2: Integer;
  ASortInfo: TcxDataSortInfo): Integer;
begin
  Result := CompareByField(ARecordIndex1, ARecordIndex2, ASortInfo.Field);
...
function TcxCustomDataController.FindRecordByFields(ABufferRecordIndex: Integer;
  ...
  begin
    ...
    for I := 0 to AFields.Count - 1 do
      if CompareByField(ARecordIndex, ABufferRecordIndex, TcxCustomDataField(AFields[I])) <> 0 then
      begin
        Result := False;
        Break;
      end;
...
function TcxCustomDataController.LoadRecord(AData: {$IFNDEF CLR}Pointer{$ELSE}TObject{$ENDIF}): Integer;
begin
  if IsProviderMode then
  ...
  begin
    Result := DataStorage.AppendRecord;
    DataStorage.ReadRecord(Result, TcxValueDefReader(AData), True);
    DoReadRecord(Result);
  end;
end;
...
begin
  ...
  if dccSorting in AChanges then
    DoSortingChanged;
  // master-detail sync
  if dccSyncMasterPost in AChanges then
  begin
    if DetailMode = dcdmClone then
