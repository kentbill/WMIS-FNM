...
{$IFDEF CLR}
function Like(const S1: string; i1, l1: Integer; const S2: string; i2, l2: Integer;
...
begin
  ...
  repeat
    ...
    begin
      ...
      while l1 > 0 do
      begin
        if Like(S1, i1, l1, S2, i2, l2, percent_char, underline_char, escape_char) then
        begin
          Result := True;
          Exit;
        end;
    ...
    if (AEscapeFlag or (c <> underline_char)) and
      (c <> S1[i1]) then
    begin
      Result := False;
      Exit;
...
begin
  {$IFDEF CLR}
  Result := Like(UpperCase(AStr), 1, Length(AStr), APatternStr, 1, Length(UpperCase(APatternStr)),
    APercent, AUnderline, #0);
  {$ELSE}
  Result := Like(PChar(UpperCase(AStr)), Length(AStr), PChar(UpperCase(APatternStr)),
