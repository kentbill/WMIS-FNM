...
type
  ...
  protected
    ...
    class function GetItemID(AItem: TObject): Integer; override;
  ...
  TcxCustomGridTableItem = class(TComponent, IcxEditRepositoryItemListener, IcxStoredObject)
  private
    FCaption: string;
    FCells: TList;
    FCellStyle: TcxCustomEditStyle;
  ...
  protected
    ...
    function GetCellStyle: TcxCustomEditStyle;
  ...
  public
    ...
    function CreateEditStyle: TcxCustomEditStyle; virtual;
    function GetDefaultValuesProvider(AProperties: TcxCustomEditProperties = nil): IcxEditDefaultValuesProvider;
  ...
  PcxGridDataCellPos = ^TcxGridDataCellPos;
  TcxGridDataCellPos = record
    GridRecord: TcxCustomGridRecord;
    Item: TcxCustomGridTableItem;
  end;
...
{$ENDIF}
  ...
  cxEditDataRegisteredRepositoryItems, cxGrid, cxGridStrs, cxGridLevel,
  cxGridDetailsSite;
...
class function TcxGridDataController.GetItemID(AItem: TObject): Integer;
begin
...
begin
  CancelEditUpdatePost;
  if (FEditingItem <> nil) and (FEditingItem.FocusedCellViewInfo <> nil) then
    with FEditingItem.FocusedCellViewInfo do
    begin
      if IsRectEmpty(Bounds) then Exit;
      FEdit.BoundsRect := EditBounds;
      InitStyle;
      TcxCustomEditAccess(FEdit).Style := Style;
    end;
end;
...
function TcxCustomGridTableController.CanAppend: Boolean;
begin
  Result := (dceoAppend in DataController.EditOperations) and
    not (dceInsert in DataController.EditState);
end;
...
begin
  ...
  try
    ...
    if Value < 0 then
    begin
      if AIsGridMode and not DataController.IsBOF then
...
begin
  ...
  {$ELSE}
    (Application.MessageBox(GetConfirmationText,
      PChar(cxGetResourceString(@scxGridDeletingConfirmationCaption)),
      [smbOK, smbCancel], smsWarning) = smbOK) then
  {$ENDIF}
    if AMultiSelect then
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if AGoForward and GridView.OptionsData.Appending and CanAppend then
        begin
...
begin
  with ViewInfo do
    if Transparent then
    {$IFDEF LINUX}
      if BackgroundBitmap = RecordsViewInfo.BackgroundBitmap then
        Self.Canvas.FillRect(BackgroundBitmapBounds, RecordsViewInfo.BackgroundBitmapBrush)
      else
...
begin
  ...
  case Key of
    ...
    VK_RETURN, VK_F2{$IFDEF VCL}, VK_PROCESSKEY{$ENDIF}:
      ...
      begin
        ...
        if Controller.FocusedItem.Editing{$IFDEF VCL} and (Key <> VK_PROCESSKEY){$ENDIF} then
          Key := 0;
      end;
//------------------------------------------------------------------------------
// Lv, zu de 增加只读模式下可以复制选定单元格的数据
//------------------------------------------------------------------------------
    ord('c'), ord('C'):
      begin
        if ssCtrl in Shift then
          if HasCells and (Controller.FocusedItem <> nil) then
            if not VarIsNull(Controller.FocusedItem.FocusedCellViewInfo.Value) then
              Clipboard.AsText:=Controller.FocusedItem.FocusedCellViewInfo.Value
      end;
//------------------------------------------------------------------------------
// Lv, zu de 增加只读模式下可以复制选定单元格的数据
//------------------------------------------------------------------------------
...
begin
  ...
  CreateItems;
{$IFDEF LINUX}
  FBackgroundBitmapBrush := TBrush.Create;
{$ENDIF}
end;
...
begin
{$IFDEF LINUX}
  FreeAndNil(FBackgroundBitmapBrush);
{$ENDIF}
  DestroyItems;
  inherited;
end;
...
{$IFDEF LINUX}
  FBackgroundBitmapBrush.Bitmap := FBackgroundBitmap;
{$ENDIF}
  if not FGridViewInfo.IsInternalUse and not FGridViewInfo.SizeCalculating then
...
procedure TcxCustomGridTableItemStyles.GetDefaultViewParams(Index: Integer; AData: Pointer;
  out AParams: TcxViewParams);
begin
  case Index of
    isContent:
      GridView.Styles.GetRecordContentParams(AData, Item, AParams);
  else
    inherited;
  end;
end;
...
function TcxCustomGridTableItem.GetCellStyle: TcxCustomEditStyle;
begin
...
function TcxCustomGridTableItem.CreateEditStyle: TcxCustomEditStyle;
begin
  Result := GetProperties.GetStyleClass.Create(nil, True) as TcxCustomEditStyle;
  if GridView.Control <> nil then
...
procedure TcxCustomGridTableViewStyles.GetDefaultViewParams(Index: Integer; AData: Pointer;
  out AParams: TcxViewParams);
begin
  inherited;
  with AParams, LookAndFeelPainter do
    case Index of
      ...
      vsInactive:
        begin
          if AData <> nil then
            with PcxGridDataCellPos(AData)^ do
...
var
  AStyle: TcxStyle;
  ADataCellPos: PcxGridDataCellPos;
begin             
  AStyle := nil;
  if (ARecord <> nil) and Assigned(FOnGetContentStyle) then
...
begin
  Result := TObject(FilterableItems[Index].ID);
end;
...
begin
  case AButtonIndex of
    ...
    NBDI_INSERT:
      Controller.CreateNewRecord(False);
    NBDI_DELETE:
