...
{$ENDIF}
  ...
  cxClasses, cxGraphics, cxLookAndFeelPainters,
...
//------------------------------------------------------------------------------
  ...
  protected
    ...
    property FullyVisible: Boolean read FFullyVisible;
  ...
  public
    ...
    procedure ChangeActiveTab(ALevel: TcxGridLevel); virtual; abstract;
    function GetHitTest(const P: TPoint): TcxCustomGridHitTest; virtual;
  ...
  with ViewInfo do
    if LookAndFeelPainter.IsDrawTabImplemented(IsVertical) then
      LookAndFeelPainter.DrawTab(Self.Canvas, Bounds, Borders,
        Text, GridCellStateToButtonState(StateForPaint), IsVertical, Params.Font,
...
begin
  FTabsViewInfo.Level.Styles.GetTabParams(Level, AParams);
end;
...
begin
  ...
  SiteViewInfo.ChangeActiveTab(FLevel);
  Result := True;
end;
...
begin
  Result := MainSize - TabsAreaOffset - (TabsRootSize - 1);
end;
...
begin
  ...
  Inc(Result, TabsAreaOffset + TabsRootSize - 1);
end;
...
begin
  Result := Bounds;
  Result.Left := Result.Right - TabsRootSize;
end;
...
begin
  Result := Bounds;
  Result.Top := Result.Bottom - TabsRootSize;
end;
...
begin
  Result := (FActiveGridView <> nil) and not FActiveGridView.IsDestroying;
end;
...
begin
  ...
  if (Result <> nil) and Result.IsDestroying then
    Result := nil;
end;
...
function TcxCustomGridDetailsSiteViewInfo.GetHitTest(const P: TPoint): TcxCustomGridHitTest;
  ...
  begin
    with ActiveGridView.Site, Result do
    begin
      X := P.X - Left;
      Y := P.Y - Top;
    end;
