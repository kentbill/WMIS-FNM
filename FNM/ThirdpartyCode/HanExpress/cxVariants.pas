...
var
  L: Integer;
  {$IFDEF CLR}Temp: TBytes;{$ENDIF}
begin
  AStream.ReadBuffer(L, SizeOf(L));
  {$IFDEF CLR}
  SetLength(Temp, L);
  AStream.ReadBuffer(Temp, L);
  S := StringOf(Temp);
  {$ELSE}
  SetLength(S, L);
...
begin
  ...
  {$IFDEF CLR}
  AStream.WriteBuffer(BytesOf(S), L);
  {$ELSE}
...
begin
  ...
  AStream.WriteBuffer(L, SizeOf(L));
  {$IFDEF CLR}
  AStream.WriteBuffer(WideBytesOf(S), L * 2);
  {$ELSE}
...
const
  ...
  {$IFDEF DELPHI5}, varError{$IFDEF DELPHI6}, varError
     {$IFDEF CLR}, varDouble{$ENDIF}
  {$ENDIF}{$ENDIF}{$ENDIF});
var
  ValType: TValueType;
...
begin
  ...
  case ValType of
    ...
    vaInt8:
      ...
      {$ELSE}
      {$IFDEF CLR}Value{$ELSE}TVarData(Value).VShortInt{$ENDIF} := ShortInt(ReadInteger);
      {$ENDIF}
    vaInt16:
      {$IFDEF CLR}Value{$ELSE}TVarData(Value).VSmallint{$ENDIF} := Smallint(ReadInteger);
    vaInt32:
      {$IFDEF CLR}Value{$ELSE}TVarData(Value).VInteger{$ENDIF} := ReadInteger;
    vaExtended:
      {$IFDEF CLR}Value{$ELSE}TVarData(Value).VDouble{$ENDIF} := ReadFloat;
    vaString, vaLString:
      Value := ReadString;
    vaFalse, vaTrue:
      {$IFDEF CLR}Value{$ELSE}TVarData(Value).VBoolean{$ENDIF} := ValType = vaTrue;
  {$IFDEF DELPHI4}
    vaWString:
      Value := ReadWideString;
    vaSingle:
      {$IFDEF CLR}Value{$ELSE}TVarData(Value).VSingle{$ENDIF} := ReadSingle;
    vaCurrency:
      {$IFDEF CLR}Value{$ELSE}TVarData(Value).VCurrency{$ENDIF} := ReadCurrency;
    vaDate:
      {$IFDEF CLR}Value{$ELSE}TVarData(Value).VDate{$ENDIF} := ReadDate;
  {$ENDIF}
  else
...
procedure WriteVariantProc(AStream: TStream; const AValue: Variant);
  ...
  begin
    ...
    begin
      WriteValue(vaInt8);
      AStream.WriteBuffer(Shortint(Value), SizeOf(Shortint));
    end
    else
    if (Value >= Low(SmallInt)) and (Value <= High(SmallInt)) then
    begin
      WriteValue(vaInt16);
      AStream.WriteBuffer(Smallint(Value), SizeOf(Smallint));
    end
    else
    begin
      WriteValue(vaInt32);
      AStream.WriteBuffer(Integer(Value), SizeOf(Integer));
    end;
  end;
  ...
  begin
    ...
    begin
      WriteValue(vaString);
      AStream.WriteBuffer(Byte(L), SizeOf(Byte));
    end else
    begin
      WriteValue(vaLString);
      AStream.WriteBuffer(Integer(L), SizeOf(Byte));
    end;
    {$IFDEF CLR}
    AStream.WriteBuffer(BytesOf(Value), L);
    {$ELSE}
...
begin
  ...
  case VType and varTypeMask of
    ...
    varString:
      WriteString(AValue);
    {$IFNDEF DELPHI6}varByte{$ELSE}varByte, varShortInt{$ENDIF}, varSmallInt, varInteger:
      WriteInteger(AValue);
    varDouble:
      WriteFloat(AValue);
    varBoolean:
...
begin
  {$IFDEF CLR}
  AStream.ReadBuffer(Temp, SizeOf(Temp));
  Value := Currency.FromOACurrency(Temp);
  {$ELSE}
  AStream.ReadBuffer(Value, SizeOf(Value));
  {$ENDIF}
end;
...
begin
  {$IFDEF CLR}
  AStream.WriteBuffer(AValue.ToOACurrency, SizeOf(TOACurrency));
  {$ELSE}
  AStream.WriteBuffer(AValue, SizeOf(AValue));
  {$ENDIF}
end;
...
begin
  {$IFDEF CLR}
  AStream.ReadBuffer(Temp, SizeOf(Temp));
  Value := TDateTime.FromOADate(Temp);
  {$ELSE}
  AStream.ReadBuffer(Value, SizeOf(Value));
  {$ENDIF}
end;
...
begin
  {$IFDEF CLR}
  AStream.WriteBuffer(AValue.ToOADate, SizeOf(TOADate));
  {$ELSE}
  AStream.WriteBuffer(AValue, SizeOf(AValue));
  {$ENDIF}
end;
...
function TcxReader.ReadString: string;
begin
  ReadStringProc(Stream, Result);
end;
...
procedure TcxWriter.WriteString(const S: string);
begin
  WriteStringProc(Stream, S);
end;
