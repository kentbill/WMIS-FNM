...
// Lv, zu de 汉化
//------------------------------------------------------------------------------
  scxUnsupportedExport = '不支持的输出类型: %1';
  scxStyleManagerKill = '样式管理器正在使用或不能释放';
  scxStyleManagerCreate = '不能创建样式管理器';

  scxExportToHtml  = '导出为网页文件(*.html)';
  scxExportToXml   = '导出为XML文档 (*.xml)';
  scxExportToText  = '导出为文本文件(*.txt)';    

  scxEmptyExportCache = '输出缓冲为空';
  scxIncorrectUnion = '不正确的单元组合';
  scxIllegalWidth = '无效的列宽';
  scxInvalidColumnRowCount = '无效的行数或列数';
  scxIllegalHeight = '非法的行高';
  scxInvalidColumnIndex = '列 %d 超出范围';
  scxInvalidRowIndex = '行 %d 超出范围';
  scxInvalidStyleIndex = '无效的样式索引 %d';

  scxExportToExcel = '导出为Excel文件 (*.xls)';
  scxWorkbookWrite = '导出Excel文件时出错';
